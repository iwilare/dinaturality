...
Dinats : (C : Category o ℓ e) → Category (o ⊔ sucℓ ℓ ⊔ e) _ (o ⊔ ℓ)
Dinats C = record
...
hommer : {C : Category ℓ ℓ ℓ}
       → Functor (CoPresheaves′ ℓ ℓ (op C ⊗ C)) (Dinats (op C ⊗ C))
hommer {C = C} = record
...
rightAdjSquash : {C : Category ℓ ℓ ℓ}
       → Functor (Dinats (op C ⊗ C)) (Dinats C)
rightAdjSquash {C = C} = record
  { F₀ = λ F → F ∘F (((vb ※ va)) ※ va ※ vb)
  ; F₁ = λ {F} {G} α →
    let module α = DinaturalTransformation α in
    let module C = Reason C in
    {! Dinaturality.Reindexing.squash α   !}
  ; identity = {!   !}
  ; homomorphism = {!   !}
  ; F-resp-≈ = {!   !}
  }
...
aggiunzia : {C : Category ℓ ℓ ℓ} → RelativeAdjoint (Dinats (op C ⊗ C)) (relativer {C = C})
aggiunzia {C = C} = record
  { L = hommer {C = C}
  ; R = rightAdjSquash {C = C}
  ; Hom-NI = record
    { F⇒G = record
      { η = λ { (F , G) → record
        { to = λ x → {!   !}
    ...
    ; F⇐G =  record
      { η = λ { (F , G) → record
        { to = λ x → record { α = λ C → {!   !} ; commute = {!   !} ; op-commute = {!   !} }
        ; cong = {!   !}
        } }
      ; commute = {!   !}
      ; sym-commute = {!   !}
