...
module _ {ℓ} {Γ : Category ℓ ℓ ℓ} where
  ...
  lambda : {A B Φ : Functor (op Γ ⊗ Γ) (Setoids ℓ ℓ)}
        → DinaturalTransformation (Functor.₀ PBP.-×- (A , Φ)) B
        → DinaturalTransformation Φ (Set.-⇨- ∘F (Functor.op A ∘F Swap ※ B))
  lambda {Φ = Φ} α = dtHelper record
    { α = λ X → record
      { to = λ a → record
        { to = λ b → α.α X $ (b , a)
        ; cong = λ x≈y → Func.cong (α.α X) (x≈y , Setoid.refl (F₀ Φ (X , X)))
        }
  ...
  lambda⁻¹⨟lambda-iso : {A B Φ : Functor (op Γ ⊗ Γ) (Setoids ℓ ℓ)}
  ...
  lambda⨟lambda⁻¹-iso : {A B Φ : Functor (op Γ ⊗ Γ) (Setoids ℓ ℓ)}
        →  (α : DinaturalTransformation (Functor.₀ PBP.-×- (A , Φ)) B)
          → lambda⁻¹ {A = A} {B = B} {Φ = Φ} (lambda {A = A} {B = B} {Φ = Φ} α) ≃ᵈ α
  lambda⨟lambda⁻¹-iso α (eq₁ , eq₂) = Func.cong (DinaturalTransformation.α α _) (eq₁ , eq₂)
  ...
  ⇨-functor-dinat : ∀ {ℓ} {Γ : Category ℓ ℓ ℓ} {A Φ F′ G′ : Functor (op Γ ⊗ Γ) (Setoids ℓ ℓ)}
    ...
    → DinaturalTransformation (Set.-⇨- ∘F (Functor.op A ∘F Swap ※ Φ))
                              (Set.-⇨- ∘F (Functor.op F′ ∘F Swap ※ G′))
  ⇨-functor-dinat {A = A} {Φ = Φ} {F′ = F′} {G′ = G′} α β = dtHelper record
    { α = λ X → record
      { to = λ f → record
